...
class MultiAgenticSwarmLogger:
    ...
    def info(self, message: str, extra: dict = None):
        ...
        if extra is None:
            extra = {}
        extra.update({"mas_event_type": "info", "mas_session_id": self.session_id})
    ...
    def debug(self, message: str, extra: dict = None):
        ...
        if extra is None:
            extra = {}
        extra.update({"mas_event_type": "debug", "mas_session_id": self.session_id})
    ...
    def warning(self, message: str, extra: dict = None):
        ...
        if extra is None:
            extra = {}
        extra.update({"mas_event_type": "warning", "mas_session_id": self.session_id})
    ...
    def error(self, message: str, extra: dict = None):
        ...
        if extra is None:
            extra = {}
        extra.update({"mas_event_type": "error", "mas_session_id": self.session_id})
    ...
    def log_function_result(
        ...
        getattr(self.logger, level.lower())(
            f"Function result: {func_name}", extra=extra
        )

    def log_llm_request(
        self, provider: str, model: str, messages: List[Dict], context: dict = None
    ):
        """Log LLM requests."""
        extra = {
    ...
    def log_swarm_operation(
        ...
        """Log swarm-level operations (FlutterSwarm, PythonSwarm, etc.)."""
        extra = {
            "mas_event_type": "swarm_operation",
            "mas_swarm": swarm_name,
            "mas_operation": operation,
            "mas_status": status,
            "mas_details": details,
            "mas_session_id": self.session_id,
        }
        getattr(self.logger, level.lower())(
            f"Swarm operation: {swarm_name} - {operation} ({status})", extra=extra
        )

    def log_tool_call_detailed(
        self,
        tool_name: str,
        agent_name: str,
        command: str = None,
        ...
        """Log detailed tool calls with all parameters and results."""
        extra = {
            "mas_event_type": "tool_call_detailed",
            "mas_tool": tool_name,
            "mas_agent": agent_name,
            "mas_command": command,
            "mas_parameters": parameters,
            "mas_result": str(result)[:2000]
    ...
    def log_agent_creation(self, agent_name: str, agent_type: str, config: dict = None):
        """Log agent creation and configuration."""
        extra = {
            "mas_event_type": "agent_creation",
            "mas_agent": agent_name,
            "mas_agent_type": agent_type,
            "mas_config": config,
            "mas_session_id": self.session_id,
        }
        self.logger.info(
            f"Agent created: {agent_name} (type: {agent_type})", extra=extra
        )
    ...
    def log_file_operation(
        ...
        """Log file operations with detailed context."""
        extra = {
            "mas_event_type": "file_operation",
            "mas_operation": operation,
            "mas_file_path": file_path,
            "mas_agent": agent_name,
            "mas_content_length": content_length,
            "mas_success": success,
            "mas_session_id": self.session_id,
        }
        level = "INFO" if success else "ERROR"
    ...
    def log_workflow_step(
        ...
        """Log detailed workflow step execution."""
        extra = {
            "mas_event_type": "workflow_step",
            "mas_workflow": workflow_name,
            "mas_step_name": step_name,
            "mas_step_number": step_number,
            "mas_agent": agent_name,
            "mas_status": status,
            "mas_input": str(input_data)[:1000] if input_data is not None else None,
            "mas_output": str(output_data)[:1000] if output_data is not None else None,
        ...
        self.logger.info(
            f"Workflow step: {workflow_name} - {step_name} ({status})", extra=extra
        )
...
def setup_comprehensive_logging(
    ...
    if log_to_file:
        ...
        log_file = (
            log_path / f"MultiAgenticSwarm_{datetime.now().strftime('%Y%m%d')}.log"
        )
        ...
        if enable_json_logs:
            json_log_file = (
                log_path / f"MultiAgenticSwarm_{datetime.now().strftime('%Y%m%d')}.json"
            )
            json_handler = RotatingFileHandler(
                json_log_file, maxBytes=max_log_size, backupCount=backup_count
            )
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.log_function_result(
                    func.__name__, result=result, execution_time=execution_time
                )
...
class LogViewer:
    ...
    def search_logs(
        self, query: str, log_type: str = "text", case_sensitive: bool = False
    ) -> List[str]:
                    ...
                    for line in f:
                        try:
                            ...
                            if event_type in ["llm_request", "llm_response"]:
                                if (
                                    provider is None
                                    or log_entry.get("mas_provider") == provider
                                ):
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        summary = {
            "total_log_files": len(log_files["text_logs"])
            + len(log_files["json_logs"]),
            "agents": set(),
            "llm_providers": set(),
            "tools_used": set(),
            "event_counts": {},
...
def search_logs(
    query: str, log_type: str = "text", case_sensitive: bool = False
) -> None:
...
def get_logs(
    lines: Optional[int] = None, format: str = "text", log_file: Optional[str] = None
    ...
    if format == "json":
        ...
        for line in log_lines:
            try:
                ...
                json_logs.append(
                    {"message": line.strip(), "level": "UNKNOWN", "timestamp": None}
                )
        return json_logs
    else:
        return log_lines
...
def view_logs(
    lines: int = 20, level: Optional[str] = None, component: Optional[str] = None
):
    """
...
def comprehensive_log_decorator(
    logger_name: str = None, log_inputs: bool = True, log_outputs: bool = True
):
    """
    Comprehensive decorator to automatically log function calls, inputs, outputs, and timing.

    This decorator provides universal logging for any function in any swarm implementation.

    Args:
        logger_name: Custom logger name. If None, uses the module name
        log_inputs: Whether to log function inputs
        log_outputs: Whether to log function outputs

    Example:
        @comprehensive_log_decorator("flutterswarm.agent")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            effective_logger_name = (
                logger_name or f"{func.__module__}.{func.__qualname__}"
            )
            logger = MultiAgenticSwarmLogger(effective_logger_name)
            ...
            context = {
                ...
                "class": getattr(func, "__self__", None).__class__.__name__
                if hasattr(func, "__self__")
                else None,
                "file": getattr(func, "__code__", {}).co_filename
                if hasattr(func, "__code__")
                else None,
                "line": getattr(func, "__code__", {}).co_firstlineno
                if hasattr(func, "__code__")
                else None,
            }

            # Log function call with detailed context
            logger.log_function_call(
                func.__name__,
                args=args if log_inputs else None,
                kwargs=kwargs if log_inputs else None,
                context=context,
            )
            ...
            try:
                ...
                logger.log_function_result(
                    func.__name__,
                    result=result if log_outputs else "<<output hidden>>",
                    execution_time=execution_time,
                )
                ...
                logger.error(
                    ...
                    extra={
                        "mas_event_type": "function_error",
                        "mas_function": func.__name__,
                        "mas_error": str(e),
                        "mas_execution_time": execution_time,
                        "mas_context": context,
                        "mas_session_id": logger.session_id,
                    },
                )
...
def comprehensive_async_log_decorator(
    logger_name: str = None, log_inputs: bool = True, log_outputs: bool = True
):
    """
    Comprehensive async decorator to automatically log async function calls, inputs, outputs, and timing.
