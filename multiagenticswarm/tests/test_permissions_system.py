...
class TestToolNodeManagerIntegration:
    ...
    def test_workflow_restricted_execution(self, setup_integration):
        ...
        result = tool_manager.execute_tool("DataAnalyst", "Calculator", planning_context, operation="multiply", a=4,
                                           b=7)
...
def test_comprehensive_permission_scenarios():
    ...
    config_data = {
        ...
        'roles': {
            'senior_analyst': {
                'allowed_tools': ['DataAPIFetcher', 'DatabaseConnector', 'DataProcessor', 'DocumentGenerator',
                                  'EmailSender']
            },
            'junior_analyst': {
    ...
    assert permissions.check_permission("SeniorAnalyst", "DataAPIFetcher", review_context)[
               0] == True  # In allowed_agents
    assert permissions.check_permission("JuniorAnalyst", "DataProcessor", review_context)[
               0] == True  # Tool not restricted
    assert permissions.check_permission("TeamManager", "DataProcessor", review_context)[
               0] == True  # Tool not restricted
