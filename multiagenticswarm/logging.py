...
Auto-initializes comprehensive logging for universal coverage across all swarms.
"""
...
def ensure_logging_initialized(
    ...
    """
    Ensure comprehensive logging is initialized exactly once.

    This function is called automatically by all MAS components to ensure
    universal logging coverage across all swarms (FlutterSwarm, PythonSwarm, etc.).
    ...
    if _logging_initialized and not force_reinit:
        return _default_config or {}

    # Initialize comprehensive logging
    config = setup_comprehensive_logging(
        verbose=verbose, log_directory=log_directory, enable_json_logs=enable_json_logs
    )
...
def setup_logging(
    ...
    """
    return ensure_logging_initialized(
        verbose=verbose, log_directory=log_directory, enable_json_logs=enable_json_logs
    )
...
def log_warning(message: str, **kwargs) -> None:
    ...
    logger.log_system_event(
        "user_warning", {"message": message, **kwargs}, level="WARNING"
    )
