...
class JSONEncoder(json.JSONEncoder):
    """Custom JSON encoder for ToolCallRequest and other objects."""

    def default(self, obj):
        if isinstance(obj, ToolCallRequest):
            return obj.to_dict()
        elif isinstance(obj, ToolCallResponse):
            return obj.to_dict()
...
class ToolCallRequest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert ToolCallRequest to dictionary for JSON serialization."""
...
class ToolCallResponse:
    ...
    def __str__(self) -> str:
        """String representation for debugging."""
        return (
            f"ToolCallResponse(id={self.id}, name={self.name}, success={self.success})"
        )
...
class BaseTool(ABC):
    ...
    async def execute(
        self, request: ToolCallRequest, agent_name: str
    ) -> ToolCallResponse:
        """
        ...
        logger.log_tool_execution(
            tool_name=self.name, agent_name=agent_name, parameters=request.arguments
        )
        ...
        try:
            ...
            if not self.can_be_used_by(agent_name):
                raise PermissionError(
                    f"Agent '{agent_name}' cannot use tool '{self.name}'"
                )
...
class FunctionTool(BaseTool):
    ...
    def __init__(
        ...
        description = (
            description or func.__doc__ or f"Function tool for {func.__name__}"
        )
