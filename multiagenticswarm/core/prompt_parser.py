...
def heuristic_parse(prompt: str) -> Dict[str, Any]:
    ...
    if re.search(r'\b(then|after|once|following)\b', prompt, re.IGNORECASE):
        remaining_agents = [a for a in agents if not phases or a not in phases[0].get("agents", [])]
        if remaining_agents:
            ...
            dependencies[seq_agent] = phases[0]["agents"] if phases and phases[0]["pattern"] == "parallel" else []
    ...
    roles = {}
    for agent in agents:
        roles[agent] = ROLE_MAPPINGS.get(agent.lower(), f"{agent} development and implementation")

    return {"phases": phases, "rules": rules, "dependencies": dependencies, "roles": roles}
