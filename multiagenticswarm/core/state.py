...
class AgentState(TypedDict):
    ...
    # - Multiple agents may decompose tasks concurrently
    # - Complete task breakdown history needed for understanding decisions
    # - Enables backtracking and task genealogy analysis
    # - No data loss when agents work in parallel
    subtasks: Annotated[List[Dict[str, Any]], add]
    # Progress percentage (0-100) per subtask
    # NOTE: task_progress uses replace behavior (Dict[str, float])
    # - Progress only moves forward, no need for historical values
    # - Latest progress percentage is what matters for coordination
    # - Monotonic increase validation handled by aggregate_progress reducer
    # - Replace behavior prevents accumulation of stale progress data
    ...
    # NOTE: Special case for agent_outputs - Currently uses Dict[str, Any] structure
    # Future enhancement consideration: Could be changed to Dict[str, List[Any]]
    # where each agent maintains a list of historical outputs for better tracking.
    # For now, keeping replace behavior to maintain backward compatibility,
    # but versioned history is available through custom merge_agent_outputs reducer
    # in state_reducers.py when needed.
    ...
    # ========== Tool Execution ==========
    # Tool execution tracking requires append behavior to maintain:
    # - Complete audit trail for compliance and debugging
    # - Pattern analysis of tool usage across agents
    # - Failure analysis from tool_errors history
    # - Correlation between tool calls and results

    # History of all tool requests made
    # Uses operator.add reducer because:
    # - Complete tool execution history needed for debugging
    # - Multiple agents may call tools simultaneously
    # - Tool usage patterns help optimize agent performance
    # - Audit requirements demand no data loss
    ...
    # Uses operator.add reducer because:
    # - Error history crucial for troubleshooting patterns
    # - Helps identify recurring issues across agents
    # - Required for debugging concurrent tool failures
    # - Enables error correlation and analysis
    tool_errors: Annotated[List[Dict[str, Any]], add]
    ...
    # Uses operator.add reducer because:
    # - Rules are discovered and accumulated during execution
    # - Multiple agents may identify coordination rules simultaneously
    # - Complete rule history enables conflict detection
    # - Supports incremental rule learning and refinement
    coordination_rules: Annotated[List[Dict[str, Any]], add]
    ...
    # ========== Memory Layers ==========
    # Memory management requires different strategies based on access patterns:
    # - Episodic memory uses append behavior for experience accumulation
    # - Shared and private memory use replace behavior for current state
    # - Working memory maintains current task context only

    # Current conversation context
    # Uses replace behavior - represents current active conversation state
    short_term_memory: Dict[str, Any]
    # Active task and working information
    # Uses replace behavior - maintains current task context only
    working_memory: Dict[str, Any]
    # Sequence of events and experiences
    # Uses operator.add reducer because:
    # - Experience history enables agent learning from past interactions
    # - Sequential event ordering crucial for context understanding
    # - Multiple agents contribute to shared experience base
    # - Supports pattern recognition and behavior adaptation
    episodic_memory: Annotated[List[Dict[str, Any]], add]
    # Information visible to all agents
    # Uses replace behavior - represents current shared state
    # NOTE: Future enhancement could add versioning for memory layers
    shared_memory: Dict[str, Any]
    # Agent-specific private information
    # Uses replace behavior - each agent's current private state
    # NOTE: Future enhancement could add versioning for memory layers
    private_memory: Dict[str, Dict[str, Any]]

    # ========== Inter-Agent Communication ==========
    # Communication tracking requires append behavior to maintain:
    # - Complete interaction history for context preservation
    # - Chronological message ordering for conversation flow
    # - Multi-agent dialogue patterns for coordination analysis
    # - Audit trails for collaboration transparency

    # Direct messages between agents
    # Uses operator.add reducer because:
    # - Inter-agent communication log essential for collaboration
    # - Message history enables context-aware responses
    # - Supports debugging of agent coordination issues
    ...
    # Uses operator.add reducer because:
    # - Help request history shows collaboration patterns
    # - Enables tracking of agent dependencies and capabilities
    # - Required for load balancing and task distribution
    # - Supports learning from past assistance interactions
    help_requests: Annotated[List[Dict[str, Any]], add]
    # System-wide announcements
    # Uses operator.add reducer because:
    # - Broadcast message history maintains system state awareness
    # - Enables all agents to catch up on missed announcements
    # - Critical for coordination in distributed agent systems
    # - Supports replay of important system events
    broadcast_messages: Annotated[List[Dict[str, Any]], add]
    ...
    # Uses operator.add reducer because:
    # - Streaming updates accumulate to build complete state picture
    # - Multiple concurrent updates need to be preserved
    # - Enables incremental state reconstruction from stream
    # - Critical for real-time state synchronization
    partial_updates: Annotated[List[Dict[str, Any]], add]
    ...
    # Debugging and monitoring require append behavior to maintain:
    # - Complete execution audit trails for compliance
    # - Error correlation across distributed agent execution
    # - Performance analysis over time
    # - System behavior understanding and optimization

    # Schema version for compatibility checking
    state_version: str
    # Step-by-step execution log
    # Uses operator.add reducer because:
    # - Complete execution history needed for debugging
    # - Enables step-by-step replay of agent interactions
    # - Required for performance analysis and optimization
    # - Supports tracing bugs across multi-agent workflows
    execution_trace: Annotated[List[Dict[str, Any]], add]
    # Error messages and stack traces
    # Uses operator.add reducer because:
    # - Complete error history essential for debugging
    # - Error patterns help identify systemic issues
    # - Required for root cause analysis across agents
    # - Enables proactive error prevention strategies
    error_log: Annotated[List[Dict[str, Any]], add]
    ...
    **kwargs: Any,
    ...
    for key, value in kwargs.items():
        if key in state:
            ...
            logger.warning(
                f"Unknown state field '{key}' provided to create_initial_state"
            )
...
def validate_state(state: AgentState, strict: bool = False) -> bool:
    ...
    if "messages" in state and isinstance(state["messages"], list):
        for i, msg in enumerate(state["messages"]):
            if not isinstance(msg, BaseMessage):
                ...
                warnings.append(
                    f"Message at index {i} has unexpected type: {type(msg).__name__}"
                )
    ...
    if strict:
        ...
        if (
            "execution_mode" in state
            and state["execution_mode"] not in VALID_EXECUTION_MODES
        ):
            errors.append(
                f"Invalid execution_mode '{state['execution_mode']}'. Valid values: {VALID_EXECUTION_MODES}"
            )
        ...
        if "workflow_pattern" in state and state["workflow_pattern"] is not None:
            if state["workflow_pattern"] not in VALID_WORKFLOW_PATTERNS:
                errors.append(
                    f"Invalid workflow_pattern '{state['workflow_pattern']}'. Valid values: {VALID_WORKFLOW_PATTERNS}"
                )
        ...
        if "agent_status" in state and isinstance(state["agent_status"], dict):
            for agent_id, status in state["agent_status"].items():
                if status not in VALID_AGENT_STATUSES:
                    errors.append(
                        f"Invalid status '{status}' for agent '{agent_id}'. Valid values: {VALID_AGENT_STATUSES}"
                    )
        ...
        if "task_progress" in state and isinstance(state["task_progress"], dict):
            for task_id, progress in state["task_progress"].items():
                if not isinstance(progress, (int, float)):
                    ...
                    errors.append(
                        f"Progress for task '{task_id}' must be between 0 and 100"
                    )
        ...
        if "tool_permissions" in state and isinstance(state["tool_permissions"], dict):
            for agent_id, tools in state["tool_permissions"].items():
                if not isinstance(tools, list):
                    errors.append(
                        f"Tool permissions for agent '{agent_id}' must be a list"
                    )
                elif not all(isinstance(tool, str) for tool in tools):
                    errors.append(
                        f"All tool names for agent '{agent_id}' must be strings"
                    )
        ...
        if "state_version" in state and state["state_version"]:
            ...
            if not re.match(version_pattern, state["state_version"]):
                warnings.append(
                    f"State version '{state['state_version']}' doesn't follow semantic versioning pattern"
                )
        ...
        if "recursion_limit" in state:
            if not isinstance(state["recursion_limit"], int):
                ...
                warnings.append(
                    f"Recursion limit {state['recursion_limit']} is very high, consider lowering"
                )
        ...
        if "stream_mode" in state and state["stream_mode"] not in valid_stream_modes:
            errors.append(
                f"Invalid stream_mode '{state['stream_mode']}'. Valid values: {valid_stream_modes}"
            )
    ...
    if "state_version" in state and state["state_version"] != SCHEMA_VERSION:
        warnings.append(
            f"State version mismatch: expected {SCHEMA_VERSION}, got {state['state_version']}"
        )
    ...
    for agent_id, tools in permissions.items():
        ...
        for i, tool in enumerate(tools):
            if not isinstance(tool, str):
                errors.append(
                    f"Tool at index {i} for agent '{agent_id}' must be a string"
                )
