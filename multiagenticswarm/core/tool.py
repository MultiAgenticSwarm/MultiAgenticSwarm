...
class Tool:
    ...
    async def execute(
        self, agent: Union[str, "Agent"], *args, **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        logger.log_tool_execution(
            ...
            parameters={"args": args, "kwargs": kwargs, "scope": self.scope.value},
        )
        ...
        try:
            ...
            if hasattr(self.func, "__call__"):
                if (
                    hasattr(self.func, "__code__")
                    and self.func.__code__.co_flags & 0x80
                ):
                    # Async function
                    logger.log_system_event(
                        "tool_async_execution", {"tool": self.name, "agent": agent_name}
                    )
                    result = await self.func(*args, **kwargs)
                else:
                    # Sync function
                    logger.log_system_event(
                        "tool_sync_execution", {"tool": self.name, "agent": agent_name}
                    )
