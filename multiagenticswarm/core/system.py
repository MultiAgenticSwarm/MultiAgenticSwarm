...
from ..utils.logger import get_logger, get_logging_config, setup_comprehensive_logging
from .agent import Agent
from .automation import Automation
from .base_tool import BaseTool, FunctionTool, ToolScope
from .task import Collaboration, Task, TaskStatus
...
try:
    from .mcp_integration import MCPClient, MCPServer, MCPTransportType
...
class System:
                ...
                for automation in self.automations.values():
                    if automation.trigger_name == trigger.name:
                        await automation.execute(
                            event, self.execution_context, self.tasks
                        )

    # Execution Methods
    async def execute_task(
        self, task_name: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    async def execute_agent(
        ...
        try:
            ...
            return {"agent_name": agent_name, "result": result, "success": True}
        except Exception as e:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        for name, tool in self.tools.items():
            try:
                ...
                tools_info[name] = {"name": name, "error": str(e)}
        ...
        return {
            ...
            "triggers": {
                name: trigger.to_dict() for name, trigger in self.triggers.items()
            },
            "automations": {
                name: automation.to_dict()
    ...
    def enable_logging(
        self, verbose: bool = False, log_directory: Optional[str] = None
    ) -> Dict[str, str]:
        """Enable comprehensive logging for the system."""
        return setup_comprehensive_logging(verbose=verbose, log_directory=log_directory)
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "agents": {"count": len(self.agents), "names": list(self.agents.keys())},
            "tools": {"count": len(self.tools), "names": list(self.tools.keys())},
            "tasks": {"count": len(self.tasks), "names": list(self.tasks.keys())},
            "triggers": {
    ...
    def register_mcp_server(
        ...
        try:
            ...
            mcp_server = MCPServer(
                name=name, host=host, port=port, transport=transport_type
            )
                ...
                for tool_name in expose_tools:
                    if tool_name in self.tools:
                        ...
                        if base_tool:
                            ...
                            logger.warning(
                                f"Tool '{tool_name}' not found or not convertible to BaseTool"
                            )
            ...
            logger.error(
                "MCP integration not available. Install required dependencies: pip install websockets aiohttp"
            )
            raise
        except Exception as e:
        ...
        try:
            ...
            if register_tools:
                ...
                for mcp_tool in mcp_tools:
                    ...
                    logger.info(
                        f"Registered MCP tool '{mcp_tool.name}' from server '{server_url}'"
                    )
