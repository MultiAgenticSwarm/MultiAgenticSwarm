            ...
            if operation == "read":
                ...
                if recursive:
                    ...
                    for root, dirs, files in os.walk(full_path):
                        ...
                        for file in files:
                            items.append(os.path.relpath(os.path.join(root, file), full_path))
                        for dir in dirs:
                            items.append(os.path.relpath(os.path.join(root, dir), full_path) + "/")
                else:
                    items = []
                ...
                dest_path = os.path.join(working_directory, dest) if not os.path.isabs(dest) else dest
    ...
    if MAS_AVAILABLE:
        return FunctionTool(
            ...
            parameters={
                "type": "object",
                "properties": {
                    "operation": {"type": "string", "description": "The file system operation to perform (e.g., 'read', 'write', 'list_dir')."},
                    "path": {"type": "string", "description": "The path to the file or directory."},
                    "content": {"type": "string", "description": "The content to write to the file (for 'write' operation)."}
                },
...
class FileSystemTool:
    ...
    def __init__(self, working_directory: str = ".", name: str = "file_system"):
        ...
        self.name = name
        self.description = "Execute file system operations"
            ...
            if recursive:
                ...
                for root, dirs, files in os.walk(full_path):
                    ...
                    for file in files:
                        items.append({
                            "name": file,
                            "path": os.path.relpath(os.path.join(root, file), full_path),
                            "type": "file"
                        })
                    for dir in dirs:
                        items.append({
                            "name": dir,
                            "path": os.path.relpath(os.path.join(root, dir), full_path),
                            "type": "directory"
                        })
            else:
                items = []
    ...
    async def exists(self, path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "exists": os.path.exists(full_path),
            "path": full_path,
            "type": "directory" if os.path.isdir(full_path) else "file" if os.path.isfile(full_path) else "unknown"
        }
