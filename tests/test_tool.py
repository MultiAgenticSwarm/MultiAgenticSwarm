...
from multiagenticswarm.core.tool import (
    Tool,
    ToolConfig,
    ToolScope,
    create_logger_tool,
...
class TestToolSchema:
    ...
    def test_get_schema(self):
        ...
        tool = Tool(
            name="MessageTool", description="Process messages", parameters=parameters
        )
...
class TestBuiltinTools:
    ...
    async def test_logger_tool_execution(self):
        ...
        result = await logger_tool.execute(
            agent, message="Important event", level="warning"
        )
    ...
    async def test_memory_tool_execution(self):
        ...
        result = await memory_tool.execute(
            agent, key="user_preference", value="dark_mode"
        )
...
class TestToolEdgeCases:
    ...
    def test_tool_without_pydantic(self):
        ...
        config = ToolConfig(name="NoPydanticTool", description="Test without pydantic")
    ...
    async def test_execution_with_complex_return_types(self):
        ...
        # Dictionary return
        dict_tool = Tool(
            name="DictTool", func=lambda: {"key": "value", "nested": {"a": 1}}
        )
    ...
    async def test_concurrent_executions(self):
        ...
        results = await asyncio.gather(
            tool.execute("Agent1"), tool.execute("Agent2"), tool.execute("Agent3")
        )
