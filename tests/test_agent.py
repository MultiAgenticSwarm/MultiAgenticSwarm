...
class TestAgentExecution:
    ...
    async def test_basic_execution(self):
        ...
        class MockProvider:
            async def execute(self, messages, context=None):
                ...
                response.content = "Hello! I'm here to help."
            ...
            def extract_tool_calls(self, response):
                return []  # No tool calls for this test
    ...
    async def test_execution_with_tool_executor(self):
        ...
        class MockProvider:
            async def execute(self, messages, context=None):
                ...
                response = SimpleNamespace()
                response.content = '{"content": "Simple response without tool calls", "reasoning": "", "tool_calls": []}'
                response.tool_calls = []
                response.json_response = {
        ...
        result = await agent.execute("Simple test", tool_executor=tool_executor)
    ...
    async def test_execution_with_context(self):
        ...
        class MockProvider:
            async def execute(self, messages, context=None):
                ...
                response = SimpleNamespace()
                response.content = '{"content": "Based on the context, the answer is 42.", "reasoning": "", "tool_calls": []}'
                response.usage = {"total_tokens": 50}
                response.tool_calls = []
                response.json_response = {
        ...
        result = await agent.execute("What's the next number?", context=context)

        assert result["success"] == True
        # The final response should just be the content from the JSON response
        assert (
            result["output"]
            == '{"content": "Based on the context, the answer is 42.", "reasoning": "", "tool_calls": []}'
        )
    ...
    async def test_execution_error_handling(self):
        ...
        with patch(
            "multiagenticswarm.core.agent.get_llm_provider"
    ...
    async def test_execution_with_max_iterations(self):
        ...
        class MockProvider:
            ...
            async def execute(self, messages, context=None):
                ...
                response = SimpleNamespace()
                response.content = '{"content": "Calling tool again...", "reasoning": "", "tool_calls": []}'
                response.tool_calls = []
                response.json_response = {
            ...
            def extract_tool_calls(self, response):
                ...
                return [
                    ToolCallRequest(
                        id=f"call_{self.call_count}", name="test_tool", arguments={}
                    )
                ]
        ...
        result = await agent.execute("Keep calling tools", tool_executor=tool_executor)
...
class TestAgentLLMProvider:
    ...
    def test_provider_configuration(self):
        ...
        agent = Agent(
            ...
            llm_provider="anthropic",
            llm_model="claude-3-opus",
            llm_config={"api_key": "test-key", "temperature": 0.5, "max_tokens": 1000},
        )
...
class TestAgentEdgeCases:
    ...
    def test_repr_method(self):
        ...
        agent = Agent(name="ReprAgent", llm_provider="openai", llm_model="gpt-4")
    ...
    async def test_execution_without_memory(self):
        ...
        with patch(
            "multiagenticswarm.llm.providers.get_llm_provider"
        ) as mock_get_provider:
            mock_provider = AsyncMock()
...
class TestAgentIntegration:
    ...
    async def test_agent_with_multiple_tools(self):
        ...
        calc_tool = FunctionTool(func=lambda a, b: a + b, name="calculator")
        ...
        class MockProvider:
            async def execute(self, messages, context=None):
                ...
                response = SimpleNamespace()
                response.content = '{"content": "I completed the multi-tool task", "reasoning": "", "tool_calls": []}'
                response.tool_calls = []
                response.json_response = {
                    "content": "I completed the multi-tool task",
                    "reasoning": "",
                    "tool_calls": [],
                }
                return response
        ...
        result = await agent.execute(
            "Calculate 10 + 20 and format it in bold", tool_executor=tool_executor
        )
